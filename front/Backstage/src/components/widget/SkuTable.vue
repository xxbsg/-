	...
	<el-table
	    ...
	    <el-table-column
	      label="操作">
	        <template slot-scope="scope">
	          <el-button
	            size="mini"
	            @click="fnPopShow(scope.row.id,scope.row.goods_id)">修改</el-button>
	          <el-button
	            size="mini"
	            type="danger"
	 ...
	 <el-dialog title="修改商品" :visible.sync="pop_show" append-to-body width="60%">
        <el-form :model="skuForm" :inline="true" status-icon ref="userForm" label-width="100px">
          ...
          <el-form-item label="SPU" prop="goods_id">
            <el-select v-model="skuForm.goods_id" size="small" @change="skuForm.specs={}">
              <el-option
...
export default {
  ...
  methods:{
    ...
    fnGetSpecs(){
      this.axios.get(cons.apis + '/goods/'+ this.skuForm.goods_id + '/specs/', {
        headers: {
          'Authorization': 'JWT ' + token
        },
        responseType: 'json',
      ...
      .then(dat=>{
	      ...
	      .then(dat=>{
		    ...
		    this.skuForm.goods_id = dat.data.goods_id;
  	...
  	submitForm(){
       ...
       this.axios.put(cons.apis + '/skus/'+this.edit_id+'/',{
              ...
              "goods_id":this.skuForm.goods_id,
